#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: UdpMessaging

#include "Basic.hpp"


SDK_NAMESPACE_START

// Enum UdpMessaging.EUdpMessageFormat
// NumValues: 0x0006
enum class EUdpMessageFormat : uint8
{
	None                                     = static_cast<uint8>(0),
	Json                                     = static_cast<uint8>(1),
	TaggedProperty                           = static_cast<uint8>(2),
	CborPlatformEndianness                   = static_cast<uint8>(3),
	CborStandardEndianness                   = static_cast<uint8>(4),
	EUdpMessageFormat_MAX                    = static_cast<uint8>(5),
};

// ScriptStruct UdpMessaging.UdpMockMessage
// 0x0010 (0x0010 - 0x0000)
struct FUdpMockMessage final
{
public:
	TArray<uint8>                                 Data;                                              // 0x0000(0x0010)(ZeroConstructor, NativeAccessSpecifierPublic)
};

SDK_NAMESPACE_END
