...
uintptr_t CGPMemoryScanner::Find_ADRL_Sig(const std::string& signature, int step) const
{
    ...
    uint32_t adrpInsn = *reinterpret_cast<uint32_t*>(insnAddress);
    uint32_t addInsn  = *reinterpret_cast<uint32_t*>(insnAddress + 0x4);
...
uintptr_t CGPMemoryScanner::Find_ADRP_LDRSTR_Sig(const std::string& signature, int step) const
{
    ...
    uint32_t adrpInsn = *reinterpret_cast<uint32_t*>(insnAddress);
    uint32_t ldrStrInsn = *reinterpret_cast<uint32_t*>(insnAddress + 0x4);
