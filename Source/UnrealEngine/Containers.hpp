...
public:
	...
	inline void CopyFrom(const TArray<ArrayElementType>& Other)
	{
...
public:
	...
	FString(const char16_t* other)
	{
		MaxElements = NumElements = *other ? std::char_traits<char16_t>::length(other) + 1 : 0;

		if (NumElements)
		{
			Data = const_cast<char16_t*>(other);
		}
	}
...
public:
	...
	inline bool Add(const TAllocatedElementType& Element)
	{
		if (this->GetSlack() <= 0)
			return false;
...
public:
	inline ValueElementType Find(const KeyElementType& Key, bool(*Equals)(const KeyElementType& LeftKey, const KeyElementType& RightKey) = [](auto R, auto L){ return R == L; })
	{
