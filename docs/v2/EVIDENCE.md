...
| V2-EV-006 | V2 packaging is revision-bound and emits a read-only manifest, raw dylib and matching dSYM. | statically validated | Build scripts and prior immutable artifacts; Gate 2A receipt is added after its clean build. |
...
| V2-EV-012 | Gate 2A selects by exact name, joint dyld-main/`MH_EXECUTE`, architecture, UUID, stable segments/fingerprint and unique candidate count. | source + synthetic static validation | [Gate 2A report](evidence/GATE2A_EXACT_IMAGE_IDENTITY.md); runtime loaded-image receipt unverified. |
