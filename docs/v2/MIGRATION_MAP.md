...
| `MenuLoad/MenuBootstrap.mm` | UIKit/menu setup and runtime initialization. | useful infrastructure | Composition root owns explicit startup result. UI must show unsupported build/contract failure without partially enabling features. |
| `MenuLoad/OverlayView.h/.mm` | Metal render hook, ImGui frame and 30 fps call to runtime Tick. | useful presentation; rejected gameplay scheduler | Presentation adapter may remain platform-specific. It may publish commands/read snapshots only. No UObject access or game lifecycle ownership from Metal thread. |
...
| vendored ImGui | UI rendering. | useful external dependency | UI-only; no engine dependency. Keep third-party code outside SourceV2 review scope unless modified. |
